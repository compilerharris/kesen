                    ...
                    if(isset($languages[$i])&&$languages[$i]!=null&&$languages[$i]!=''){
                        ...
                        if(isset($rateCard)){
                            ...
                            EstimatesDetails::updateOrCreate([
                                ...
                                'lang' => $languages[$i],
                                'unit' => $request->type == "customize" ? 1 : ($request->type == "minimum"?1:$request['unit'][$index]),
                                'rate' => $request->type == "customize" ? $rateCard->customize_rate : ($request->type == "minimum"?$rateCard->t_minimum_rate:$rateCard->t_rate),
                            ], [
                                'estimate_id' => $estimate->id,
                                'document_name' => $document_name,
                                'type' => $request->type,
                                ...
                                'verification' => $request->type == "customize" ? null : (isset($request['v_one']) && is_array($request['v_one']) && isset($request['v_one'][$index]) && $request['v_one'][$index] === 'on' ? ($request->type == "minimum"?$rateCard->v1_minimum_rate:($request['unit'][$index]*$rateCard->v1_rate)) : null),
                                'v2' => isset($request['v_two']) && is_array($request['v_two']) && isset($request['v_two'][$index]) && $request['v_two'][$index] === 'on' ? true : false,
                                'two_way_qc_t' => $request->type == "customize" ? null : (isset($request['v_two']) && is_array($request['v_two']) && isset($request['v_two'][$index]) && $request['v_two'][$index] === 'on' ? ($request->type == "minimum"?$rateCard->v2_minimum_rate:($request['unit'][$index]*$rateCard->v2_rate)) : null),
                                'bt' => isset($request['bt']) && is_array($request['bt']) && isset($request['bt'][$index]) && $request['bt'][$index] === 'on' ? true : false,
                                'back_translation' => isset($request['bt']) && is_array($request['bt']) && isset($request['bt'][$index]) && $request['bt'][$index] === 'on' ? ($request->type == "customize" ? $rateCard->customize_rate : ($request->type == "minimum"?$rateCard->bt_minimum_rate:$rateCard->bt_rate)) : null,
                                'btv' => isset($request['btv']) && is_array($request['btv']) && isset($request['btv'][$index]) && $request['btv'][$index] === 'on' ? true : false,
                                'verification_2' => $request->type == "customize" ? null : (isset($request['btv']) && is_array($request['btv']) && isset($request['btv'][$index]) && $request['btv'][$index] === 'on' ? ($request->type == "minimum"?$rateCard->btv_minimum_rate:($request['unit'][$index]*$rateCard->btv_rate)) : null),
