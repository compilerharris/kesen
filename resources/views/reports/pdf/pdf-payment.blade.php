            ...
            @foreach ($job_card as $job)
                @if($job->t_unit != '' && $job->t_unit != 0 && is_numeric($job->t_unit) && $job->t_writer_code == $writer_payment->writer_id)
                    <tr>
                        ...
                        @php if($job->estimateDetail)$total+=Modules\WriterManagement\App\Models\WriterLanguageMap::where('writer_id',$writer_payment->writer_id)->where('language_id',$job->estimateDetail->language->id)->first()->per_unit_charges*$job->t_unit @endphp
                    </tr>
                    @endif
                @if($job->v_unit != '' && $job->v_unit != 0 && is_numeric($job->v_unit) && $job->v_employee_code == $writer_payment->writer_id)
                    <tr>
                        ...
                        @php if($job->estimateDetail)$total+=Modules\WriterManagement\App\Models\WriterLanguageMap::where('writer_id',$writer_payment->writer_id)->where('language_id',$job->estimateDetail->language->id)->first()->checking_charges*$job->v_unit @endphp
                    </tr>
                @endif
                @if($job->bt_unit != '' && $job->bt_unit != 0 && is_numeric($job->bt_unit) && $job->bt_writer_code == $writer_payment->writer_id)
                    <tr>
                        ...
                        @php if($job->estimateDetail)$total+=Modules\WriterManagement\App\Models\WriterLanguageMap::where('writer_id',$writer_payment->writer_id)->where('language_id',$job->estimateDetail->language->id)->first()->bt_charges*$job->bt_unit @endphp
                    </tr>
                @endif
                @if($job->btv_unit != '' && $job->btv_unit != 0 && is_numeric($job->btv_unit) && $job->btv_employee_code == $writer_payment->writer_id)
                    <tr>
                        ...
                        @php if($job->estimateDetail)$total+=Modules\WriterManagement\App\Models\WriterLanguageMap::where('writer_id',$writer_payment->writer_id)->where('language_id',$job->estimateDetail->language->id)->first()->bt_checking_charges*$job->btv_unit @endphp
                    </tr>
                @endif
            @endforeach
